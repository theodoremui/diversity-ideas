    ...
    if url is not None and len(url) > 0:
        while attempts <= numRetries and len(content) < 5:
            ...
            if html is not None and len(html) > 10:
                ...
                if (
                    (dateObj is None or len(DATE_SELECTOR.strip()) == 0)
                    and URL_DATE_PATTERN is not None
                    and len(URL_DATE_PATTERN.strip()) > 0
                ):
                    try:
...
def getArticleList(
    ...
    while attempts <= numRetries and len(articleList) == 0:
        ...
        if showProgress:
            print(f"\tretrieved: {len(articleList)}")
        attempts += 1
    ...
    for pageNumber in pageList:
        ...
        if len(articleList) == 0:
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [
                    executor.submit(process_article, article) for article in articleList
                ]
...
def startProcessing(startPage: int, endPage: int, numRetries: int) -> None:
    ...
    df, failedPages = getArticles(LISTING_BASE_URL, pageNumbers, showProgress=True)
